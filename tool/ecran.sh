...
#   PAQUET=fr.bevannes.bevannes.demo bash tool/ecran.sh lancer   la démo
...
D="$(cd "$(dirname "$0")" && pwd)"
mkdir -p "$D/captures"
PAQUET="${PAQUET:-fr.bevannes.bevannes}"
...
case "$1" in
  ...
  lancer)
    ...
    "$ADB" shell am start -n "$PAQUET/fr.bevannes.bevannes.MainActivity" > /dev/null
    sleep "${2:-6}" ;;
