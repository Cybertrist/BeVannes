...
.w{width:${W}px;height:${H}px;display:flex;align-items:center;gap:22px;padding:0 30px 0 22px;
   border-radius:18px;border:1.5px solid #1C4A43;
   background:linear-gradient(100deg,#0C2420 0%,#081916 55%,#050F0D 100%)}
...
.f{width:58px;height:58px;border-radius:16px;flex-shrink:0;display:flex;align-items:center;justify-content:center;
   background:linear-gradient(135deg,#A7F3E6,#39D2C0)}
</style></head><body><div class="w">
  <div class="i"><img src="file:///$B/../../assets/icon/icon.png"></div>
  <div class="t">
    <b>$(t 'Télécharger BeVannes' 'Download BeVannes')</b>
    <span>v$VERSION · Android · $(t 'mode démo' 'demo mode')$TAILLE</span>
  </div>
  <div class="f"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#02221E"
...
"$CH" --headless=new --disable-gpu --hide-scrollbars --allow-file-access-from-files --virtual-time-budget=10000 \
  ...
  --screenshot="$OUTW/telecharger.png" "file:///$B/html$SUF/telecharger.html" >/dev/null 2>&1
